...
public class CanonicalResourceManager<T extends CanonicalResource> {
  ...
  public void copy(CanonicalResourceManager<T> source) {
    ...
    // matchbox patch https://github.com/ahdis/matchbox/issues/538: the copy has to keep the whole index, otherwise
    // the definitions of the master (core) package are not preferred anymore when resolving an unversioned canonical
    // and the latest version wins (e.g. an R5 CodeSystem from hl7.fhir.uv.xver-r5.r4 in an R4 context)
